...
def getContiguousAreas(data, value, min_pixels=1, contiguity="queen"):
    ...
    assert contiguity in [
        "rook",
        "queen",
    ], "Contiguity must be either 'rook' or 'queen'. Input recieved was <%s>." % str(
        contiguity
    )
    ...
    location_id_unique, location_id_indices = np.unique(
        location_id, return_inverse=True
    )
    location_id = np.arange(0, location_id_unique.shape[0], 1)[
        location_id_indices
    ].reshape(data.shape)
...
def _calculatePatchSize(tile, patch_size):
    ...
    assert (
        patch_size == "auto" or type(patch_size) == float or type(patch_size) == int
    ), "Patch size must be numeric or set to 'auto'."

    # Calculate patch_size (which aims for a 100 x 100 output image if set to auto.)
    if patch_size == "auto":
        patch_size = int(round(((tile.xSize + tile.ySize) / 2.0) / 100.0))

    return patch_size


def _buildOutputArray(tile, patch_size, dtype=int):
    """
    ...
    output_size = int(round(((tile.xSize + tile.ySize) / 2.0) / patch_size))
...
def _getBlocks(tile, array):
    ...
    for n, ymin in enumerate(
        np.linspace(0, tile.ySize, array.shape[0], endpoint=False)
    ):
        ymax = ymin + (tile.ySize / array.shape[0])
        for m, xmin in enumerate(
            np.linspace(0, tile.xSize, array.shape[1], endpoint=False)
        ):
            xmax = xmin + (tile.xSize / array.shape[1])

            ns.append(n)
            ms.append(m)
            ymins.append(int(round(ymin)))
            ymaxs.append(int(round(ymax)))
            xmins.append(int(round(xmin)))
            xmaxs.append(int(round(xmax)))
...
def calculateLDI(tile, patch_size="auto", output=False, show=False):
    ...
    def _computeLDI(unique_ids):
        ...
        if (mask_selection == False).sum() > 0:
            LDI = int(
                round(
                    (float(different_patches) / (mask_selection == False).sum()) * 100
                )
            )
    ...
    for n, m, ymin, ymax, xmin, xmax in _getBlocks(tile, LDI):
        ...
        _, nonforest_id = getContiguousAreas(
            this_patch, False, contiguity=tile.contiguity
        )

        # Supply a unique ID to each habitat patch, whether forest or nonforest.
        unique_ids = forest_id.copy()
        unique_ids[nonforest_id != 0] = forest_id[nonforest_id != 0] + (
            nonforest_id[nonforest_id != 0] + np.max(forest_id) + 1
        )
    ...
    if output:
        biota.IO.outputGeoTiff(
            LDI,
            tile.output_pattern % "LDI",
            tile.shrinkGeoT(patch_size),
            tile.proj,
...
def calculateLDIChange(tile_change, patch_size="auto", output=False, show=False):
    ...
    if output:
        biota.IO.outputGeoTiff(
            LDI_change,
            tile_change.output_pattern % "LDIChange",
            tile_change.shrinkGeoT(patch_size),
            tile_change.proj,
            output_dir=tile_change.output_dir,
            dtype=gdal.GDT_Int32,
            nodata=tile_change.nodata,
        )
...
def calculateTWC(tile, patch_size="auto", output=False, show=False):
    ...
    for n, m, ymin, ymax, xmin, xmax in _getBlocks(tile, TWC):
        ...
        if WC.mask.sum() <= ((patch_size**2) * 0.5):
            ...
            TWC.data[n, m] = int(
                round((float(WC.sum()) / ((patch_size**2) - WC.mask.sum())) * 100)
            )
...
def calculateWCC(tile_change, patch_size="auto", output=False, show=False):
    ...
    if output:
        biota.IO.outputGeoTiff(
            WCC,
            tile_change.output_pattern % "WCC",
            tile_change.shrinkGeoT(patch_size),
            tile_change.proj,
            output_dir=tile_change.output_dir,
            dtype=gdal.GDT_Int32,
            nodata=tile_change.nodata_byte,
        )
...
def calculateProportionalChange(
    tile_change, change_type, patch_size="auto", output=False, show=False
):
    """
    ...
    for n, m, ymin, ymax, xmin, xmax in _getBlocks(tile_change, change_downsampled):
        ...
        if C.mask.sum() <= ((patch_size**2) * 0.5):
            ...
            change_downsampled.data[n, m] = int(
                round((float(C.sum()) / ((patch_size**2) - C.mask.sum())) * 100)
            )
    ...
    if output:
        biota.IO.outputGeoTiff(
            change_downsampled,
            tile_change.output_pattern % "%sDownsampled" % change_type.title(),
            tile_change.shrinkGeoT(patch_size),
            tile_change.proj,
            output_dir=tile_change.output_dir,
            dtype=gdal.GDT_Int32,
    ...
    if show:
        biota.IO.showFigure(
            ...
            tile_change.lat,
            tile_change.lon,
            title="%s Downsampled" % change_type.title(),
            cbartitle="%",
            vmin=0,
            vmax=50,
            cmap="Spectral_r",
