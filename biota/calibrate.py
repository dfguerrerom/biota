...
def extractGamma0(
    ...
    assert (
        year >= 2007 and year <= 2010
    ) or year >= 2015, "Invalid year (%s) input" % str(year)
    ...
    if agb_field is not None:
        agb = biota.mask.getField(shp, agb_field).astype(float)
    ...
    for lat, lon in tiles:
        ...
        # Mask out each plot
        plot_mask = biota.mask.maskShapefile(
            tile, shp, location_id=True, buffer_size=buffer_size
        )
        ...
        for n in np.unique(plot_mask[plot_mask != 0]):
            ...
            if np.isnan(np.nanmean(data_gamma0_hv[this_mask])):
                pdb.set_trace()
    ...
    with open(
        "gamma0_%s_by_plot.csv" % str(year), "wb"
