...
class LoadTile(object):
    ...
    def __init__(
        ...
        assert lat < 90.0 or lat > -90.0, "Latitude must be between -90 and 90 degrees."
        ...
        assert (year >= 2007 and year <= 2010) or (
            year >= 2015 and year <= dt.datetime.now().year
        ), (
            "Years must be in the range 2007 - 2010 and 2015 - present. Your input year was %s."
            % str(year)
        )
        assert (
            downsample_factor >= 1 and type(downsample_factor) == int
        ), "Downsampling factor must be an integer greater than 1."
        assert (
            type(lee_filter) == bool
        ), "Option lee_filter must be set to 'True' or 'False'."
        ...
        assert contiguity in [
            "rook",
            "queen",
        ], "Contiguity constraint must be 'rook' or 'queen'."
        assert os.path.isdir(
            os.path.expanduser(data_dir)
        ), "Specified data directory (%s) does not exist" % str(data_dir)
        assert os.path.isdir(
            os.path.expanduser(sm_dir)
        ), "Specified soil moisture directory (%s) does not exist" % str(sm_dir)
        assert sm_interpolation in [
            "nearest",
            "average",
            "cubic",
        ], "Soil moisture interpolation type must be one of 'nearest', 'average' or 'cubic'."
        assert os.path.isdir(
            os.path.expanduser(output_dir)
        ), "Specified output directory (%s) does not exist" % str(output_dir)
        assert (
            type(forest_threshold) == float or type(forest_threshold) == int
        ), "Forest threshold must be numeric."
        assert (
            type(area_threshold) == float or type(area_threshold) == int
        ), "Area threshold must be numeric."
        ...
        self.AGB_parameters_path = (
            self.__getAGBParametersPath()
            if parameter_file == None
            else os.path.expanduser(parameter_file)
        )  #
        assert os.path.exists(
            self.AGB_parameters_path
        ), "Specified parameter file (%s) not found." % str(self.AGB_parameters_path)
    ...
    def __getDirectory(self):
        ...
        directory = (
            self.data_dir
            + "/"
            + name_pattern % (lat_dir, lon_dir, str(self.year)[-2:], "MOS")
            + "/"
        )
        ...
        if not os.path.isdir(directory):
            ...
            if not os.path.isdir(directory):
                raise IOError(
                    "No tile for lat: %s, lon: %s exists in the specified data directory."
    ...
    def __getHHPath(self):
        ...
        if not os.path.isfile(HHpath):
            raise IOError(
                "No data found for HH polarisation for lat: %s, lon: %s."
                % (str(self.lat), str(self.lon))
            )
    ...
    def __getHVPath(self):
        ...
        if not os.path.isfile(HVpath):
            raise IOError(
                "No data found for HV polarisation for lat: %s, lon: %s."
                % (str(self.lat), str(self.lon))
            )
    ...
    def __getOutputPattern(self):
        ...
        return "%s_%s_%s%s.tif" % (
            "%s",
            str(self.year),
            self.hem_NS + str(abs(self.lat)).zfill(2),
            self.hem_EW + str(abs(self.lon)).zfill(3),
        )
    ...
    def shrinkGeoT(self, downsample_factor):
        ...
        geo_t[1] = 1.0 / int(round(self.xSize / float(downsample_factor)))
        geo_t[-1] = (1.0 / int(round(self.ySize / float(downsample_factor)))) * -1
    ...
    def __rebin(self):
        ...
        native_resolution = (self.xRes + self.yRes) / 2.0
    ...
    def __getMask(self, masked_px_count=False, output=False, show=False):
        ...
        if self.downsample_factor != 1 and not masked_px_count:
            mask = skimage.measure.block_reduce(
                mask, (self.downsample_factor, self.downsample_factor), np.sum
            ) >= ((self.downsample_factor**2) * 0.25)
        ...
        if output:
            self.__outputGeoTiff(mask, "Mask", dtype=gdal.GDT_Byte)

        if show:
            self.__showArray(mask, title="Mask", cmap="coolwarm")

        return mask

    def updateMask(
        self, filename, buffer_size=0.0, classes=[], output=False, show=False
    ):
        """
        ...
        # Load mask
        mask = biota.mask.updateMask(
            self, filename, buffer_size=buffer_size, classes=classes
        )
        ...
        if output:
            self.__outputGeoTiff(self.mask * 1, "Mask", dtype=gdal.GDT_Byte)

        if show:
            self.__showArray(self.mask * 1, title="Mask", cmap="coolwarm")
    ...
    def loadAGBParameters(self, csv_file):
        ...
        assert csv_file.endswith(".csv"), (
            "Specified parameter file (%s) must be a .csv." % csv_file
        )
        assert os.path.exists(os.path.expanduser(csv_file)), (
            "Specified parameter file (%s) doesn't exist." % csv_file
        )
    ...
    def getDN(self, polarisation="HV", output=False, show=False):
        ...
        assert (
            polarisation == "HH" or polarisation == "HV"
        ), "polarisation must be either 'HH' or 'HV'."
        ...
        if self.downsample_factor != 1:
            # Load the sum of DNs
            DN_sum = skimage.measure.block_reduce(
                DN, (self.downsample_factor, self.downsample_factor), np.sum
            )

            # Load the sum of contributing pixels
            block_sum = skimage.measure.block_reduce(
                np.ones_like(DN),
        ...
        if output:
            self.__outputGeoTiff(DN, "DN", dtype=gdal.GDT_Int32)

        if show:
            self.__showArray(
                DN, title="DN", cbartitle="Digital Number", cmap="Spectral_r"
            )
    ...
    def __getDay(self):
        ...
        if self.downsample_factor != 1:
            day_after_launch = skimage.measure.block_reduce(
                day_after_launch,
                (self.downsample_factor, self.downsample_factor),
                np.max,
            )
    ...
    def getDate(self, output=False, show=False):
        ...
        if not hasattr(self, "date"):
            ...
            unique_dates = [
                launch_date + dt.timedelta(days=int(d)) for d in unique_days
            ]
            ...
            for day, date in zip(unique_days, unique_dates):
                ...
                dates_int[day_after_launch == day] = int(
                    np.datetime64(date, "D").astype(dt.date).strftime("%Y%m%d")
                )
        ...
        if output:
            self.__outputGeoTiff(self.dates_int, "Date", dtype=gdal.GDT_Int32)

        if show:
            print(
                "Sorry, matplotlib doesn't support display of numpy datetime objects right now."
            )
    ...
    def getDOY(self, output=False, show=False):
        ...
        if not hasattr(self, "DOY"):
            ...
            unique_doys = [
                (d.astype(dt.date) - dt.date(d.astype(dt.date).year, 1, 1)).days + 1
        ...
        if output:
            self.__outputGeoTiff(self.DOY, "DOY", dtype=gdal.GDT_Int32)
    ...
    def getYearArray(self, output=False, show=False):
        ...
        if output:
            self.__outputGeoTiff(self.YearArray, "YearArray", dtype=gdal.GDT_Int32)
    ...
    def getSM(self, output=False, show=False, search_days=7):
        ...
        if not hasattr(self, "SM"):
            SM = biota.SM.getSM(
                self, search_days=search_days, interpolation=self.SM_interpolation
            )
        ...
        if output:
            self.__outputGeoTiff(self.SM, "SM")
    ...
    def getGamma0(self, polarisation="HV", units="natural", output=False, show=False):
        ...
        assert units == "natural" or units == "decibels", (
            "Units must be 'natural' or 'decibels'. You input %s." % units
        )
        assert polarisation == "HH" or polarisation == "HV", (
            "Polarisation must be 'HH' or 'HV'. You input %s." % polarisation
        )
        ...
        if self.lee_filter:
            gamma0 = biota.filter.enhanced_lee_filter(
                gamma0, n_looks=self.nLooks, window_size=self.window_size
            )

        # Convert to natural units where specified
        if units == "natural":
            gamma0 = 10 ** (gamma0 / 10.0)
        ...
        if output:
            self.__outputGeoTiff(gamma0, "Gamma0")

        if show:
            # Different display settings depending on options
            if polarisation == "HH" and units == "natural":
                vmin, vmax = 0, 0.15
            if polarisation == "HV" and units == "natural":
                vmin, vmax = 0, 0.06
            if polarisation == "HH" and units == "decibels":
                vmin, vmax = -15, -5
            if polarisation == "HV" and units == "decibels":
                vmin, vmax = -20, -10
    ...
    def getAGB(self, output=False, show=False):
        ...
        if not hasattr(self, "AGB"):
            ...
            if self.satellite == "ALOS-1":
                AGB = (
                    self.A1_gradient
                    * self.getGamma0(units="natural", polarisation="HV")
                    + self.A1_intercept
                )
            ...
            # ALOS-2 (to calculate)
                ...
                raise ValueError(
                    "Unknown satellite named '%s'. self.satellite must be 'ALOS-1' or 'ALOS-2'."
                    % self.satellite
                )
        ...
        if output:
            self.__outputGeoTiff(self.AGB, "AGB")

        if show:
            self.__showArray(
                self.AGB, title="AGB", cbartitle="tC/ha", vmin=0, vmax=40, cmap="YlGn"
            )
    ...
    def getWoodyCover(self, output=False, show=False):
        ...
        if not hasattr(self, "WoodyCover"):
            ...
            if self.area_threshold > 0:
                ...
                min_pixels = int(
                    round(self.area_threshold / (self.yRes * self.xRes * 0.0001))
                )

                # Remove pixels that aren't part of a forest block of size at least min_pixels
                contiguous_area, _ = biota.indices.getContiguousAreas(
                    WoodyCover, True, min_pixels=min_pixels, contiguity=self.contiguity
                )
    ...
    def getForestPatches(self, output=False, show=False):
        ...
        if not hasattr(self, "ForestPatches"):
            ...
            _, ForestPatches = biota.indices.getContiguousAreas(
                WoodyCover, True, min_pixels=min_pixels, contiguity=self.contiguity
            )
        ...
        if output:
            self.__outputGeoTiff(
                self.ForestPatches * 1, "ForestPatches", dtype=gdal.GDT_Int32
            )
    ...
    def __showArray(
        self, data, title="", cbartitle="", vmin=None, vmax=None, cmap=None
    ):
        """
...
class LoadChange(object):
    ...
    def __init__(
        ...
        tile_t1,
        tile_t2,
        change_intensity_threshold=0.2,
        change_magnitude_threshold=0.0,
        ...
        if self.deforestation_threshold is None:
            self.deforestation_threshold = self.tile_t1.forest_threshold
    ...
    def __testTiles(self):
        ...
        assert (
            self.tile_t1.lat == self.tile_t2.lat
            and self.tile_t1.lon == self.tile_t2.lon
        ), "Input tiles must be from the same location."
        assert (
            self.tile_t1.year <= self.tile_t2.year
        ), "Input tile_t2 must be from a later year than tile_t1."
        assert (
            self.tile_t1.year != self.tile_t2.year
        ), "Input tile_t1 cannot be from the same year as tile_t2."
        assert (
            self.tile_t1.lee_filter == self.tile_t2.lee_filter
        ), "Only one of the input tiles has been filtered. Both tiles should have the same pre-processing parameters."
        ...
        assert (
            self.tile_t1.xSize == self.tile_t2.xSize
            and self.tile_t1.ySize == self.tile_t2.ySize
        ), "Input tiles do not have the same resolution."
        assert (
            self.tile_t1.geo_t == self.tile_t2.geo_t
        ), "Input tiles do not have the same geo_transform."
        assert (
            self.tile_t1.forest_threshold == self.tile_t2.forest_threshold
        ), "'forest_threshold' must be identical for both input tiles."
        assert (
            self.tile_t1.area_threshold == self.tile_t2.area_threshold
        ), "'area_threshold' must be identical for both input tiles."
        assert (
            self.tile_t1.downsample_factor == self.tile_t2.downsample_factor
        ), "'downsample_facor' must be identical for both input tiles."
    ...
    def getSMChange(self, output=False, show=False):
        ...
        if output:
            self.__outputGeoTiff(self.SMChange, "SMChange")
    ...
    def getGamma0Change(
        self, polarisation="HV", units="natural", output=False, show=False
    ):
        """ """

        # Always run processing as pol/units may change
        self.Gamma0_change = self.tile_t2.getGamma0(
            polarisation=polarisation, units=units
        ) - self.tile_t1.getGamma0(polarisation=polarisation, units=units)

        # Add combined mask
        self.Gamma0_change.mask = self.mask

        if output:
            self.__outputGeoTiff(self.Gamma0_change, "Gamma0Change")
    ...
    def getAGBChange(self, output=False, show=False):
        ...
        if output:
            self.__outputGeoTiff(self.AGB_change, "AGBChange")
    ...
    def getChangeType(self, output=False, show=False):
        ...
        if not hasattr(self, "ChangeType"):
            ...
            F_NF = np.logical_and(
                self.tile_t1.getWoodyCover(), self.tile_t2.getWoodyCover() == False
            )
            NF_F = np.logical_and(
                self.tile_t1.getWoodyCover() == False, self.tile_t2.getWoodyCover()
            )

            # Pixels that remain forest (F_F) or nonforest (NF_NF)
            F_F = np.logical_and(
                self.tile_t1.getWoodyCover(), self.tile_t2.getWoodyCover()
            )
            NF_NF = np.logical_and(
                self.tile_t1.getWoodyCover() == False,
                self.tile_t2.getWoodyCover() == False,
            )

            # Get pixels of change greater than intensity threshold
            CHANGE_INTENSITY = np.logical_or(
                (self.getAGBChange() / self.tile_t1.getAGB())
                >= self.change_intensity_threshold,
                (self.getAGBChange() / self.tile_t1.getAGB())
                < (-self.change_intensity_threshold),
            )

            # Get pixels of change greater than magnitude threshold
            CHANGE_MAGNITUDE = np.logical_or(
                self.getAGBChange() >= self.change_magnitude_threshold,
                self.getAGBChange() < (-self.change_magnitude_threshold),
            )
            ...
            if self.change_area_threshold > 0:
                min_pixels = int(
                    round(self.change_area_threshold / (self.yRes * self.xRes * 0.0001))
                )

                # Get areas of change that meet minimum area requirement (use 'change' pixels for area measurement)
                if self.combine_areas:
                    CHANGE_INCREASE, _ = biota.indices.getContiguousAreas(
                        CHANGE & INCREASE & (NF_F | F_F),
                        True,
                        min_pixels=min_pixels,
                        contiguity=self.contiguity,
                    )
                    CHANGE_DECREASE, _ = biota.indices.getContiguousAreas(
                        CHANGE & DECREASE & (F_NF | F_F),
                        True,
                        min_pixels=min_pixels,
                        contiguity=self.contiguity,
                    )
                    ...
                    CHANGE_DEF, _ = biota.indices.getContiguousAreas(
                        CHANGE & DECREASE & F_NF,
                        True,
                        min_pixels=min_pixels,
                        contiguity=self.contiguity,
                    )
                    CHANGE_DEG, _ = biota.indices.getContiguousAreas(
                        CHANGE & DECREASE & F_F,
                        True,
                        min_pixels=min_pixels,
                        contiguity=self.contiguity,
                    )
                    CHANGE_GRO, _ = biota.indices.getContiguousAreas(
                        CHANGE & INCREASE & F_F,
                        True,
                        min_pixels=min_pixels,
                        contiguity=self.contiguity,
                    )
                    CHANGE_AFF, _ = biota.indices.getContiguousAreas(
                        CHANGE & INCREASE & NF_F,
                        True,
                        min_pixels=min_pixels,
                        contiguity=self.contiguity,
                    )
                    CHANGE = np.logical_or(
                        np.logical_or(CHANGE_DEF, CHANGE_DEG),
                        np.logical_or(CHANGE_GRO, CHANGE_AFF),
                    )
                    NOCHANGE = CHANGE == False
            ...
            change_code = (
                np.zeros((self.ySize, self.xSize), dtype=np.int8) + self.nodata_byte
            )
    ...
    def getRiskMap(self, output=False, show=False, buffer_size=50.0):
        """
        ...
        if self.deforestation_threshold != self.tile_t1.forest_threshold:
            change_type_noDF = LoadChange(
                self.tile_t1,
                self.tile_t2,
                change_intensity_threshold=self.change_intensity_threshold,
                change_magnitude_threshold=self.change_magnitude_threshold,
                change_area_threshold=self.change_area_threshold,
                deforestation_threshold=None,
                contiguity=self.contiguity,
                output_dir=self.output_dir,
            ).getChangeType()
        ...
        # Low risk of change
        low_dilate = scipy.ndimage.morphology.binary_dilation(
            (np.logical_or(deforestation == 1, deforestation_noDF == 1)).astype(
                np.int8
            ),
            iterations=int(
                round(buffer_size / ((self.tile_t1.xRes + self.tile_t1.yRes) / 2.0), 0)
            ),
        )  # 50 m buffer
        ...
        if output:
            self.__outputGeoTiff(self.risk_map, "RiskMap")

        if show:
            self.__showArray(
                self.risk_map,
                title="Deforestation risk map",
                cbartitle="Low - High risk",
                vmin=0,
                vmax=3,
                cmap="autumn",
            )
    ...
    def getAGBSum(self, proportion=False, output=False):
        ...
        if proportion:
            for change_type in totals:
                totals[change_type] = (
                    totals[change_type]
                    / (self.tile_t1.getAGB() * self.xRes * self.yRes * 0.0001).sum()
                )
