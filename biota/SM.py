...
def _findFiles(tile, date, search_days=7, data_source="CCI"):
    ...
    assert (
        data_source == "CCI" or data_source == "SMAP"
    ), "Soil moisture data_source must be CCI or SMAP."
    ...
    assert len(data_files) > 0, (
        "No data found in soil moisture data path (%s)." % data_files
    )
    ...
    if data_source == "CCI":
        data_dates = np.array(
            [
                dt.datetime.strptime(
                    data_file.split("/")[-1].split("-")[-2][:8], "%Y%m%d"
                ).date()
        ...
        data_dates = np.array(
            [
                dt.datetime.strptime(
                    data_file.split("/")[-1].split("_")[-3], "%Y%m%d"
                ).date()
                for data_file in data_files
            ]
        )
    ...
    sel = np.logical_and(
        data_dates >= date - dt.timedelta(search_days),
        data_dates <= date + dt.timedelta(search_days),
    )

    assert len(data_files[sel]) > 0, (
        "No data for tile date found in soil moisture data path (%s)." % tile.SM_dir
    )
...
def _readSM(sm_file, data_source="CCI"):
    ...
    if data_source == "CCI":
        ...
        # Load dataset
        ds = gdal.Open(
            'HDF5:"%s"://Soil_Moisture_Retrieval_Data_AM/soil_moisture' % sm_file
        )

        # Build geo_t equivalent
        latitudes = gdal.Open(
            'HDF5:"%s"://Soil_Moisture_Retrieval_Data_AM/latitude' % sm_file
        ).ReadAsArray()
        longitudes = gdal.Open(
            'HDF5:"%s"://Soil_Moisture_Retrieval_Data_AM/longitude' % sm_file
        ).ReadAsArray()
...
def _readTimeStack(tile, date, search_days=7, data_source="CCI"):
    ...
    for n, sm_file in enumerate(sm_files):
        ...
        yCount = int(abs(math.floor(1.0 / latDist)) + 1)
        xCount = int(math.ceil(1.0 / lonDist) + 1)

        this_sm = ds.ReadAsArray(xOffset, yOffset, xCount, yCount)

        if "sm_out" not in locals():
            sm_out = np.ma.zeros(
                (this_sm.shape[0], this_sm.shape[1], (search_days * 2) + 1),
                dtype=np.float32,
            )

        # Both datasets have -9999. as a nodata value
        sm_out[:, :, n] = np.ma.array(this_sm, mask=this_sm == -9999.0)

        # Close dataset
        ds = None

    geo_t_out = (
        ulLon + (xOffset * lonDist),
        geo_t[1],
        geo_t[2],
        ulLat + (yOffset * latDist),
        geo_t[4],
        geo_t[5],
    )
        ...
        for j in range(sm.shape[1]):
            ...
            if np.sum(this_sm.mask == False) > 1:
                f = scipy.interpolate.interp1d(
                    x[this_sm.mask == False],
                    y[this_sm.mask == False],
...
def _resampleSM(sm_interp, tile, geo_t, interpolation="avearge"):
    ...
    ds_source = gdal_driver.Create(
        "", sm_interp.shape[0], sm_interp.shape[1], 1, gdal.GDT_Float32
    )
    ...
    return np.ma.array(sm_resampled, mask=sm_resampled == -9999.0)
...
def _loadSM(tile, date, search_days=7, interpolation="average", data_source="CCI"):
    ...
    sm, geo_t = _readTimeStack(
        tile, date, search_days=search_days, data_source=data_source
    )
...
def getSM(tile, search_days=7, interpolation="average"):
    ...
    assert interpolation in [
        ...
        "nearest",
        "cubic",
    ], "Soil moisture interpolation type must be 'average', 'nearest', or 'cubic'."

    # Build an array of dates in string format, which is quicker to search
    dates = tile.getDate().astype(int)

    # Generate output array
    out = np.zeros_like(tile.mask, dtype=np.float32) + 999999.0

    for date in np.unique(dates):  # np.unique(tile.getDate()).astype(int):
        ...
        if len(glob.glob(tile.SM_dir + "/ESACCI-SOILMOISTURE-*.nc")) > 0:
            ...
            assert False, (
                "No data from CCI or SMAP soil moisture products found in sm_dir (%s)"
                % tile.SM_dir
            )

        # Interpolate through time and draw out the central value, and zoom to scale of tile
        sm_resampled = _loadSM(
            tile,
            dt.datetime(1970, 1, 1).date() + dt.timedelta(date),
        ...
        if (
            float((sm_resampled.mask[dates == date]).sum())
            / (sm_resampled.mask[dates == date]).shape[0]
            <= 0.25
        ):
    ...
    # Add the mask
    out = np.ma.array(out, mask=np.logical_or(tile.mask, out == 999999.0))
